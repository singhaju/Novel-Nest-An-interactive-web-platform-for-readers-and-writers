### Brief goal
Help contributors implement features and fix bugs in Novel Nest — a Next.js 13 (App Router) fullstack app with Prisma-backed Postgres, server and client components, and REST-style API routes under `app/api`.
...
- Prisma is the ORM: the singleton client is in `lib/prisma.ts`. DB migrations & studio: `pnpm prisma:migrate` and `pnpm prisma:studio` (see `package.json` scripts).
...
- Prisma usage: import `prisma` from `lib/prisma` (already built as a singleton to avoid multiple clients in dev). Do not new-up PrismaClient directly.
...
- `package.json` — scripts you can run: `pnpm dev` (next dev), `pnpm build` (runs `prisma generate && next build`), and Prisma helpers.
...
- If server-side DB code fails, check `lib/prisma.ts` usage and ensure `DATABASE_URL` is set.
...
- `lib/prisma.ts` — prisma singleton pattern to reuse.
